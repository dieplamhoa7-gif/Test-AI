"""
STEP 3 (Claude fix) â€” Walk-forward combo 5-8 indicator family / task

Má»¥c tiÃªu: ghÃ©p tá»• há»£p 5-8 family Ä‘Ã£ Ä‘Æ°á»£c Step 2 fix chá»©ng minh cÃ³ lower-bound CI cao,
huáº¥n luyá»‡n model trÃªn combo, Ä‘Ã¡nh giÃ¡ qua walk-forward vá»›i purged gap, threshold tune
trÃªn VAL, calibration isotonic, bootstrap CI, Bonferroni penalty máº¡nh hÆ¡n (vÃ¬ combo
nhÃ¢n sá»‘ config), vÃ  yÃªu cáº§u pass cáº£ 3 tiÃªu chÃ­:
   (1) lower bound CI precision â‰¥ 70%
   (2) tá»•ng predN â‰¥ 50 qua N folds
   (3) expected net pnl/trade > 0 sau 0.4% phÃ­ round-trip

KhÃ¡c biá»‡t vá»›i step3 gá»‘c:
  1. Äá»c Step 2 Claude-fix (file _Claude_fix.json).
  2. KHÃ”NG dÃ¹ng "best per family" tháº³ng â€” chá»n top N family theo lower-bound CI
     cá»§a Step 2 (conservative), bá» family cÃ³ expected_pnl Ã¢m.
  3. Walk-forward CV vá»›i purged gap.
  4. Má»i threshold Ä‘á»u tune trÃªn VAL.
  5. Probability calibration isotonic.
  6. Bootstrap 95% CI; rank theo lower bound.
  7. Bonferroni penalty máº¡nh: tá»•ng sá»‘ combo cÃ³ thá»ƒ lÃªn Ä‘áº¿n vÃ i trÄƒm.
  8. Sample size guard cháº·t: tá»•ng predN â‰¥ 50.
  9. Output thÃªm trÆ°á»ng "tradePlan": vá»›i combo top 1 per task, gá»£i Ã½ entry/stop/target
     dá»±a trÃªn S/R hiá»‡n táº¡i (náº¿u task lÃ  SUPPORT_HOLD/RESISTANCE_REJECT/RS).

File nÃ y KHÃ”NG Ä‘Ã¨ file gá»‘c.
"""
...
# Import step2 fix module dynamically Ä‘á»ƒ re-use load_rows / label / folds / clf
STEP2_PY = ROOT / "step2_tune_params_precision_first_rs_d1a_d1s_Claude_fix_run.py"
...
# Walk-forward config (Ä‘á»“ng bá»™ step2 fix)
N_FOLDS = step2.N_FOLDS
HORIZON_DAYS = step2.HORIZON_DAYS
VAL_RATIO = step2.VAL_RATIO
MIN_TOTAL_PRED = 10            # nghiÃªm hÆ¡n step1/step2 vÃ¬ step3 quÃ©t combo
MIN_FOLD_PRED = 3
...
MODELS = ["ET", "RF", "LOG"]
GROUP_SIZES = [2, 3, 4, 5]
MAX_POOL_PER_TASK = 8   # tá»‘i Ä‘a 8 family pool â†’ C(8,5)+...+C(8,8) = 93 combo/task
...
def eval_combo(rows, folds, task, combo, model_name, n_total_configs):
    ...
    bonferroni_penalty = math.log(max(n_total_configs, 2)) * 1.2  # step3 penalty máº¡nh nháº¥t
...
def main():
    if not STEP2.exists():
        raise FileNotFoundError(f"KhÃ´ng tÃ¬m tháº¥y {STEP2}. Cháº¡y step2 Claude-fix trÆ°á»›c.")
    ...
    # Pool family per task: chá»‰ giá»¯ family cÃ³ expected_pnl > 0 vÃ  CI lower bound cao,
    # 1 variant tá»‘t nháº¥t / family, tá»‘i Ä‘a MAX_POOL_PER_TASK
    pools = {}
    for task, arr in s2.items():
        seen = set()
        pool = []
        # Sort láº¡i Ä‘á»ƒ cháº¯c cháº¯n theo conservative rank
        arr_sorted = sorted(arr, key=lambda x: (x["precisionCI95LowerMean"], x["avgPrecision"],
                                                x["totalPredN"]), reverse=True)
        for r in arr_sorted:
            if r["family"] in seen:
                continue
            if r.get("precisionCI95LowerMean", 0) < 0:
                continue
    ...
    print(f"[info] tá»•ng combo sáº½ quÃ©t: {total_configs}", flush=True)

    runs = []
    for task, pool in pools.items():
        if len(pool) < min(GROUP_SIZES):
            print(f"  {task}: pool quÃ¡ nhá» ({len(pool)}) â†’ bá» qua", flush=True)
            continue
        combos = []
        for k in GROUP_SIZES:
            if k <= len(pool):
    ...
    # TÃ³m táº¯t: cho má»—i task láº¥y top 1 conservative-pass náº¿u cÃ³
    top_per_task = {}
    ...
    payload = {
        ...
        "method": (
            "STEP 3 Claude-fix: Walk-forward combo 5-8 family/task; pool family Ä‘Ã£ filter "
            "theo expected_pnl > 0 vÃ  CI lower bound â‰¥ 50 tá»« Step 2 fix. "
            f"N_FOLDS={N_FOLDS}, purged gap={HORIZON_DAYS}d, isotonic calibration, "
            "threshold tune trÃªn VAL, bootstrap CI, rank theo lower bound, "
