...
OUT=Path('data/v1_method_a_swing_profit_optimization.json')
LOOKBACK_DAYS=240; OOS_DAYS=80
HORIZONS=[20,30,42]
GRIDS=[]
for horizon in HORIZONS:
 for entry_pad in [0.02,0.03,0.04,0.05]:
  for stop_mode in ['support_pad','entry_pct']:
   for stop_val in [0.03,0.04,0.05,0.06]:
    for target_mode in ['fixed_1_5r','fixed_2r','fixed_2_5r','fixed_3r','pct_6','pct_8','pct_10','res_or_2r']:
     for rsi_min in [30,35,40]:
      for vol_max in [3.5,99.0]:
       GRIDS.append(dict(horizon=horizon,entry_pad=entry_pad,stop_mode=stop_mode,stop_val=stop_val,target_mode=target_mode,rsi_min=rsi_min,vol_max=vol_max))

def enrich(df):
 df=df.copy()
 for c in ['open','high','low','close','volume']: df[c]=pd.to_numeric(df[c],errors='coerce')
 df['vol20']=df.volume.rolling(20).mean(); df['ma20']=df.close.rolling(20).mean(); df['ma50']=df.close.rolling(50).mean(); df['ma200']=df.close.rolling(200).mean()
...
def precompute(max_horizon=max(HORIZONS)):
 ...
 for sym in UNIVERSE:
  try:
   ...
   for i in range(90,len(df)-max_horizon-1):
    ...
    rows.append({'symbol':sym,'date':str(row.time.date()),'price':price,'support':support,'resistance':resistance,'rsi':float(row.rsi) if pd.notna(row.rsi) else 0,'volRatio':vol/vol20 if vol20 else 1,'hist':float(row.macdHist) if pd.notna(row.macdHist) else 0,'ma20':float(row.ma20) if pd.notna(row.ma20) else 0,'ma50':float(row.ma50) if pd.notna(row.ma50) else 0,'ma200':float(row.ma200) if pd.notna(row.ma200) else 0,'future':future[['high','low','close','time']].assign(time=future['time'].astype(str)).to_dict('records')}); cnt+=1
...
def trade_for(row,cfg):
 ...
 # reject only broken long trend, allow wider pullback
 if row['ma50']>0 and p<row['ma50']*0.90: return None
 if cfg['stop_mode']=='support_pad': stop=s*(1-cfg['stop_val'])
 else: stop=p*(1-cfg['stop_val'])
 risk=p-stop
 if risk<=0: return None
 tm=cfg['target_mode']
 if tm=='fixed_1_5r': target=p+risk*1.5
 elif tm=='fixed_2r': target=p+risk*2
 elif tm=='fixed_2_5r': target=p+risk*2.5
 elif tm=='fixed_3r': target=p+risk*3
 elif tm=='pct_6': target=p*1.06
 elif tm=='pct_8': target=p*1.08
 elif tm=='pct_10': target=p*1.10
 else:
  target=max(res or 0, p+risk*2)
 rr=(target-p)/risk
...
def main():
 rows=precompute(); split=pd.Timestamp(datetime.now()-timedelta(days=OOS_DAYS)); results=[]
 for cfg in GRIDS:
  ...
  penalty=0
  if os['totalTrades']<10: penalty+=40
  if ss['sumPnlPct']<0: penalty+=abs(ss['sumPnlPct'])*0.2
  if os['winRatePct']<35 and os['totalTrades']>0: penalty+=10
  score=os['sumPnlPct'] + os['avgPnlPct']*8 + min(os['totalTrades'],60)*0.25 + ss['sumPnlPct']*0.10 - penalty
  results.append({'config':cfg,'score':round(score,2),'sample':ss,'oos':os,'all':allsum})
 results.sort(key=lambda x:x['score'], reverse=True)
 best_cfg=results[0]['config']; best_trades=[t for r in rows for t in [trade_for(r,best_cfg)] if t]
 payload={'createdAt':datetime.now().isoformat(),'method':'V1 Method A swing optimization: longer hold, wider stop, higher target; output file for reuse; exclude VIC/VHM','lookbackDays':LOOKBACK_DAYS,'sampleDays':LOOKBACK_DAYS-OOS_DAYS,'oosDays':OOS_DAYS,'gridCount':len(GRIDS),'topResults':results[:40],'bestTrades':best_trades}
 OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8')
 print(json.dumps(results[:10],ensure_ascii=True,indent=2)); print('saved',OUT)
