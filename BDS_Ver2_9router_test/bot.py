...
async def cmd_gia(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Em ?ang tra ?o?n ???ng theo NQ87/K1 t? t?a ??, kh?ng d?ng ngu?n rao b?n th? tr??ng...")
    try:
        report = await asyncio.to_thread(lookup_nq87_k1_report, lat, lng, "ODT", "VT1")
        await update.message.reply_text(report)
    except Exception as e:
        logger.exception("K1 lookup failed")
        await update.message.reply_text(f"Kh?ng tra ???c NQ87/K1 cho t?a ?? n?y: {e}")


def lookup_nq87_k1_report(lat: float, lng: float, land_use: str = "ODT", position: str = "VT1") -> str:
    """Call the shared NQ87/K1 lookup from the planning bot. /gia must use official K1 road segment, not market listing sources."""
    root = Path(__file__).resolve().parents[1]
    cli = root / "bds_k1_cli.js"
    cp = subprocess.run(
        ["node", str(cli), str(lat), str(lng), land_use, position],
        cwd=str(root),
        text=True,
        encoding="utf-8",
        errors="replace",
    ...
    if cp.returncode != 0:
        raise RuntimeError((cp.stderr or out or f"node exit {cp.returncode}").strip()[:1000])
    data = json.loads(out)
    if not data.get("ok"):
        raise RuntimeError(data.get("error") or "lookup failed")
    k1 = data.get("k1") or {}
    m = k1.get("match") or {}
    c = k1.get("calc") or {}
    ...
    lines = [
        "? Tra NQ87/K1 theo t?a ??",
        f"T?a ??: {lat}, {lng}",
        f"Geocode: {loc.get('display_name') or ''}",
        "",
        f"Ph??ng/x? ph? l?c: {m.get('wardHeader') or '?'}",
        f"???ng: {m.get('road') or '?'}",
        f"?o?n ?p gi?: {m.get('segment') or '?'}",
        f"Trang ngu?n: {m.get('page') or '?'} | ?? tin c?y: {m.get('confidence') or 'medium'}",
        "",
        f"??t ? b?ng gi?: {float(m.get('priceResidentialThousand') or c.get('baseThousandPerM2') or 0):,.0f} ngh?n/m?",
        f"TMDV: {float(m.get('priceCommercialThousand') or 0):,.0f} ngh?n/m?",
        f"SXKD: {float(m.get('priceBusinessThousand') or 0):,.0f} ngh?n/m?",
        f"K ??t ?: {m.get('kResidential') or c.get('marketK') or 0}",
        f"??n gi? ?i?u ch?nh ??t ?: {float(c.get('adjustedMillionPerM2') or 0):,.3f} tri?u/m?",
        "",
        "L?u ?: ??y l? ?o?n t? file NQ87/K1 theo rule bot quy ho?ch, kh?ng ph?i ngu?n rao b?n th? tr??ng. V? tr? 1/2/3/4 v?n c?n ki?m tra th?c ??a/h? s?.",
    ]
    if alts:
        lines += ["", "C?c ?o?n ?ng vi?n kh?c ?? ??i chi?u:"]
        for i, a in enumerate(alts[:5], 1):
            lines.append(f"{i}. {a.get('road')} | {a.get('segment')} | trang {a.get('page')} | score {a.get('score')}")
    return "\n".join(lines)[:3900]
