...
function parseGulandPopupText(text) {
  ...
  if (!out.planning.length && (m = t.match(new RegExp(`\\b(${codes})\\b\\s*(?:[\\d.,]+\\s*m(?:2|²)?)?\\s*([^]+?)(?=\\s+Thông tin quy hoạch|\\s+Dữ liệu chính|$)`, 'i')))) {
    out.planning.push({ code: m[1].toUpperCase(), land_use: (m[2] || '').trim(), area_m2: out.parcel.area_m2 || null });
  }
  ...
  if ((m = t.match(/Thông tin quy hoạch xây dựng\s*([\d.,]+)\s*m(?:2|²)?\s*([^]+?)(?=\s+Dữ liệu chính|\s+Xem đầy đủ|$)/i))) {
    ...
    const parts = [];
    const firstUse = first.replace(/\s*Tổng\s*:.*$/i, '').trim();
    ...
    if (traffic) parts.push({ code: 'DGT', land_use: traffic[2], area_m2: normalizeNumber(traffic[1]), kind: 'construction_planning' });
    out.planning.push(...parts);
  }

  if ((m = t.match(/Tầng\s*:\s*([^;]+?)(?:;|\s*Mặt|$)/i))) out.height = m[1].trim();
  if ((m = t.match(/Mặt tiền\s*:\s*([\d.,]+)\s*%?/i))) out.density = normalizeNumber(m[1]);  // note: may be density or frontage, adjust if needed
