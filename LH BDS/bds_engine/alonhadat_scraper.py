...
def _property_keywords(criteria: SearchCriteria) -> list[str]:
    ...
    if mode == "rent":
        if rent_subtype == "rent_vanphong": return ["cho thuê văn phòng", "thuê văn phòng"]
        if rent_subtype == "rent_santhuongmai": return ["cho thuê mặt bằng", "thuê mặt bằng kinh doanh"]
        if rent_subtype == "rent_chungcu" or pt == "chungcu": return ["cho thuê căn hộ", "cho thuê chung cư"]
        if pt == "khoxuong": return ["cho thuê kho xưởng", "thuê kho xưởng"]
        return ["cho thuê nhà", "thuê nhà", "cho thuê nhà mặt tiền"]
...
def _parse_listing_text(text: str, url: str, mode: str = "buy") -> Listing | None:
    ...
    if not text or any(x in low for x in ["cần mua", "can mua", "tuyển", "ký gửi"]):
        return None
    if mode == "buy" and ("/tháng" in low or "/thang" in low or "/năm" in low or "/nam" in low or "cho thuê" in low or "cho thue" in low):
        return None
    if mode == "rent":
        rent_markers = ["cho thuê", "cho thue", "cần cho thuê", "can cho thue", "thuê", "thue", "/tháng", "/thang", "triệu/tháng", "trieu/thang"]
        if not any(x in low for x in rent_markers):
            return None
        sale_markers = ["cần bán", "can ban", "bán nhà", "ban nha", "bán đất", "ban dat", "tỷ", "ty"]
        if any(x in low for x in sale_markers) and not any(x in low for x in ["/tháng", "/thang", "triệu/tháng", "trieu/thang"]):
            return None
    price_total = None
    ...
    if mode == "rent":
        if ppm <= 0.01 or ppm > 5:
            return None
    elif ppm <= 1 or ppm > 1500:
        return None
...
def _candidate_queries(criteria: SearchCriteria, project_name: str | None = None) -> list[str]:
    loc = getattr(criteria, "location_context", {}) or {}
    district = (loc.get("district") or loc.get("county") or loc.get("city_district")) if isinstance(loc, dict) else None
    city = loc.get("city") if isinstance(loc, dict) else None
    ward = (loc.get("ward") or loc.get("suburb") or loc.get("phuong")) if isinstance(loc, dict) else None
    ...
    for kw in kws[:3]:
        parts=[kw, street, ward, district, city]
        bases.append(" ".join(x for x in parts if x))
        parts2=[kw, ward, district, city]
        bases.append(" ".join(x for x in parts2 if x))
        parts3=[kw, district, city]
        bases.append(" ".join(x for x in parts3 if x))
    # de-dupe
    out=[]; seen=set()
    for q in bases:
...
def scrape_alonhadat_query(query: str, criteria: SearchCriteria, limit: int = 10) -> list[Listing]:
    mode=getattr(criteria,'transaction','buy') or 'buy'
    path = 'cho-thue-nha-dat' if mode == 'rent' else 'can-ban-nha-dat'
    url = f"{BASE}/{path}?kw={quote_plus(query)}"
    ...
    for a in soup.select('a[href$=".html"]'):
        ...
        if mode == 'rent' and not any(x in (text or '').lower() for x in ['cho thuê','cho thue','thuê','thue','/tháng','/thang','triệu/tháng','trieu/thang']): continue
        if mode != 'rent' and not ('Giá:' in text or re.search(r'\d+[\.,]?\d*\s*tỷ',text,re.I)): continue
        if mode == 'rent' and not ('Giá:' in text or re.search(r'\d+[\.,]?\d*\s*triệu\s*/?\s*tháng',text,re.I)): continue
