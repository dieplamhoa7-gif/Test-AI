...
### Automatic skill rules

- For long Telegram/OpenClaw work where context gets slow/confused, use task-specific handoffs instead of one global context. Read `context/TASKS.md` to identify the task, then read only `context/tasks/<task-id>.md` plus relevant project files. If no task exists, create one from `context/TASK_TEMPLATE.md`. Never mix unrelated jobs into the same task file. `CURRENT_CONTEXT.md` is legacy/simple fallback only.
