...
OUT = Path('data/wave_entry_base_6m_target20_h60_backtest.json')
EXCLUDE = {'VIC','VHM'}
HORIZON = 60
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'name':'Wave Entry after 6m base accumulation target20 H60','method':'Uses long history from 2022-01-01. User requested horizon reduced from 84 to 60 sessions. Exit: target +20%, take 50% at +20%, trailing stop for remaining 50% at max(entry+1%, peak-8%), initial stop 8%, horizon 60 sessions. Output saved; web should only read cache.','windows':{'current180':current,'prev3m':prev}}
