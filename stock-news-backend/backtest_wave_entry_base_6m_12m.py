...
OUT = Path('data/wave_entry_base_6m_12m_backtest.json')
EXCLUDE = {'VIC','VHM'}
HORIZON = 42
...
def pass_strategy(name, ai):
    ...
    if name=='WaveA_base_breakout_safe':
        if not (base6_ok or base12_ok): return False,'no_6m_12m_base'
    ...
    if name=='WaveC_strong_momentum_after_base':
        if not base12_ok: return False,'no_12m_base'
    ...
    for sym in TECHNICAL_UNIVERSE[:50]:
        ...
        for i in range(252,len(df)-HORIZON-2):
            ...
            for strat in strategies:
                ...
                tr=trade_manage(df,i,target_pct=8,stop_pct=6,trail_pct=4)
                if tr:
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'name':'Wave Entry after 6m-12m base accumulation','method':'Looks for stocks forming a 6-12 month base, then entering wave by breakout/early momentum/strong momentum. Output saved; web should only read cache. Exit: target +8%, take 50%, trailing 4%, stop 6%.','windows':{'current180':current,'prev3m':prev}}
