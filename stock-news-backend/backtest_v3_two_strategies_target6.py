...
OUT=Path('data/v3_two_strategies_target6_backtest.json')
EXCLUDE={'VIC','VHM'}
HORIZON=42
...
def pass_strategy(name,rs,ai):
    ...
    if name=='A4_bottom_reversal_rsi_low':
        # True bottom/reversal setup: RSI low, near support, positive divergence + MACD recovery.
        # Allow in_cloud/above_cloud, reject below_cloud because user does not want buying after lower-cloud breakdown.
        if ichi=='below_cloud': return False,'below_cloud_no_buy'
        if dist>2.5: return False,'dist>2.5'
        if rsi>40: return False,'rsi>40'
        if bbp>0.45: return False,'bbp>0.45'
        if volr<0.55 or volr>2.5: return False,'vol_bad'
        if roc<-12: return False,'roc<-12'
        if not ai.get('bullishDivergence'): return False,'no_bullish_divergence'
        if not ai.get('macdHistRecovering'): return False,'macd_not_recovering_3'
        return True,'ok'
    if name=='B4_trend_pullback':
        # Trend pullback, not bottom fishing.
        if ichi!='above_cloud': return False,'not_above_cloud'
        if dist>3.0: return False,'dist>3'
        if not (48<=rsi<=62): return False,'rsi_not_48_62'
        if volr<0.55 or volr>2.2: return False,'vol_bad'
        if roc<-8 or roc>12: return False,'roc_bad'
        if not (ai.get('macdHistRecovering') or (ai.get('bullishDivergence') and mh>=-0.05)): return False,'no_macd_recovery_or_div'
        if bbp>0.85: return False,'bbp_too_high'
        return True,'ok'
...
def run_window(name,start,end,histories):
    strategies=['A4_bottom_reversal_rsi_low','B4_trend_pullback']
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'method':'Two clean-split strategies, fixed exit target +6%, take 50%, trailing, stop 6%. A4_bottom_reversal: RSI<=40 near support, bullish RSI/MACD divergence, MACD hist recovering 3 bars, no below_cloud. B4_trend_pullback: above_cloud, RSI 48-62, MACD recovering or bullish divergence. R/S remains zone-only. Excludes VIC/VHM. Saves full entryIndicators and futurePath.','windows':{'current180':current,'prev3m':prev}}
