...
import pandas as pd
from app.market_data import _compute_indicators, _detect_momentum_divergence, Quote
from app.technical_filters import TECHNICAL_UNIVERSE

OUT = Path('data/wave_entry_base_6m_target20_long_history_backtest.json')
EXCLUDE = {'VIC','VHM'}
HORIZON = 84
...
def load_long_history(symbol: str, start: str = '2022-01-01'):
    if Quote is None:
        return None
    df = Quote(symbol=symbol, source='VCI').history(start=start, end=datetime.now().strftime('%Y-%m-%d'), interval='1D')
    if not isinstance(df, pd.DataFrame) or df.empty:
        return None
    rename_map={'datetime':'time','date':'time','Date':'time','open':'open','high':'high','low':'low','close':'close','volume':'volume'}
    df=df.rename(columns={k:v for k,v in rename_map.items() if k in df.columns})
    need=['time','open','high','low','close','volume']
    if not all(c in df.columns for c in need):
        return None
    work=df[need].copy(); work['time']=pd.to_datetime(work['time']);
    for c in ['open','high','low','close','volume']:
        work[c]=pd.to_numeric(work[c], errors='coerce')
    return work.dropna(subset=['time','open','high','low','close']).sort_values('time').reset_index(drop=True)

def load_histories(symbols):
    out={}
    for idx,sym in enumerate(symbols,1):
        try:
            df=load_long_history(sym); out[sym]=df; print('history',idx,sym,0 if df is None else len(df),flush=True)
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'name':'Wave Entry after 6m base accumulation target20 long history','method':'Uses long history from 2022-01-01 instead of app default 2025-01-01 to provide enough data for 6m base + OOS + target20. Exit: target +20%, take 50% at +20%, trailing stop for remaining 50% at max(entry+1%, peak-8%), initial stop 8%, horizon 84 sessions. Output saved; web should only read cache.','windows':{'current180':current,'prev3m':prev}}
