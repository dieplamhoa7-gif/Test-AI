...
OUT=Path('data/v3_clean_split_rs_action_relaxed_a2_b2_backtest.json')
EXCLUDE={'VIC','VHM'}
HORIZON=42
...
def pass_strategy(name,rs,ai):
    ...
    if name=='A2_oversold_near_support_relaxed':
        if dist>2.5: return False,'dist>2.5'
        if rsi>45: return False,'rsi>45'
        if bbp>0.55: return False,'bbp>0.55'
        if volr<0.55 or volr>2.5: return False,'vol_bad'
        if roc<-12: return False,'roc<-12'
        if not (improving or mh>=-0.08 or ai.get('bullishDivergence')): return False,'macd_not_improving'
        return True,'ok'
    if name=='B2_confirmed_rebound_above_cloud_relaxed':
        ...
        if volr<0.55 or volr>2.5: return False,'vol_bad'
        if roc<-8 or roc>15: return False,'roc_bad'
        if not (mh>=-0.05 or improving): return False,'macd_bad'
        if bbp>0.9: return False,'bbp_too_high'
        return True,'ok'
...
def run_window(name,start,end,histories):
    strategies=['A2_oversold_near_support_relaxed','B2_confirmed_rebound_above_cloud_relaxed']
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'method':'Clean split relaxed A2/B2. R/S only produces support/resistance/zone/dist/stop-target references. Action uses RSI/MACD/BB/volume/ROC/Ichimoku/divergence only. A2: dist<=2.5 RSI<=45 BB<=0.55. B2: above_cloud dist<=3 RSI 48-62. Excludes VIC/VHM. Saves full entryIndicators and futurePath.','windows':{'current180':current,'prev3m':prev}}
