...
def eval_strategy(name,price,rs,ai):
    ...
    if name=='A2_oversold_near_support':
        ...
        labels=['kh├┤ng thß╗ºng m├óy d╞░ß╗¢i','gß║ºn hß╗ù trß╗ú <=2.5%','RSI <=45','BB thß║Ñp <=0.55','volume vß╗½a','ROC20 kh├┤ng qu├í xß║Ñu','MACD hist hß╗ôi dß║ºn 3 phi├¬n','kh├┤ng bearish divergence']
        ...
        labels=['above cloud','gß║ºn hß╗ù trß╗ú <=3%','RSI 48-62','volume vß╗½a','ROC20 hß╗úp lß╗ç','MACD hist hß╗ôi dß║ºn 3 phi├¬n','BB kh├┤ng qu├í cao','kh├┤ng bearish divergence']
        ok=sum(1 for x in checks if x)
        passed=all(checks)
        if not passed: reasons=[labels[i] for i,x in enumerate(checks) if not x]
    ...
    return {'strategy':name,'action':action,'rankScore':r(score),'entryPrice':entry,'lastClose':r(price),'stopLoss':r(entry*0.94),'takeProfit':r(entry*(1+target_pct/100)),'targetPct':target_pct,'stopPct':6,'distSupportPct':r(dist),'support':r(sup),'resistance':r(res),'reasonsOk':[] if not passed else ['─æß║ít ─æß╗º ─æiß╗üu kiß╗çn'], 'missingReasons':reasons,'entryIndicators':ai,'rsSnapshot':{'supportZoneDay':rs.get('supportZoneDay'),'resistanceZoneDay':rs.get('resistanceZoneDay'),'activeSupportDay':rs.get('activeSupportDay'),'activeResistanceDay':rs.get('activeResistanceDay')}}
