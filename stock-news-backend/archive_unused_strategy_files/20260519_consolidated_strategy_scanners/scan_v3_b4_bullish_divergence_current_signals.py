...
def eval_b4(price,rs,ai):
    ...
    checks=[
        ...
        (dist<=3,'gß║ºn hß╗ù trß╗ú <=3%'),
        (48<=f(ai['rsi'])<=62,'RSI 48-62'),
        (0.55<=f(ai['volumeRatio'])<=2.2,'volume vß╗½a'),
        (-8<=f(ai['roc20'])<=12,'ROC20 hß╗úp lß╗ç'),
        ((ai.get('macdHistRecovering') or (ai.get('bullishDivergence') and f(ai['macdHist'])>=-0.05)),'MACD hß╗ôi dß║ºn hoß║╖c ph├ón kß╗│ d╞░╞íng'),
        (f(ai['bbPercent'])<=0.85,'BB kh├┤ng qu├í cao'),
        (not ai.get('bearishDivergence'),'kh├┤ng bearish divergence'),
    ]
