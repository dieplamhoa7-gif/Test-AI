...
OUT=Path('data/lh_fast_selective_v2_2023_summary.json')
...
def exit_trade(df,i,target=4.0,stop=6.0,horizon=15,minhold=3):
    ...
    return (exitp-entry)/entry*100,bars,outcome
    ...
    for sym,df0 in hist.items():
        ...
        for i in range(200,len(df)-45):
            ...
            for s in ['LH2','LH1','LH3','LH4']:
                ...
                break
    payload={'createdAt':pd.Timestamp.now().isoformat(),'method':'SELECTIVE V2 fast. Uses original fast signals but reduces trades with liquidity/volatility/trend quality gates, longer cooldown, one prioritized signal per symbol/date. Exit adjusted for higher hit-rate: LH1/LH2/LH3 target4 stop6 horizon15; LH4 target6 stop6 horizon30. Research approximation only, not canonical production.','strategyMap':{'LH1':'Pullback','LH2':'Shakeout Rebound','LH3':'Support Rebound Hunter','LH4':'Wave Entry'},'windows':{}}
