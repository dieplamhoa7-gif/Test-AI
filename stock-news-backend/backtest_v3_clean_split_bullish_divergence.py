...
OUT=Path('data/v3_clean_split_bullish_divergence_backtest.json')
EXCLUDE={'VIC','VHM'}
HORIZON=42
...
def pass_strategy(name,rs,ai):
    ...
    if name=='A4_oversold_near_support_bullish_div':
        ...
        if not (ai.get('macdHistRecovering') or mh>=-0.03): return False,'macd_not_recovering'
        return True,'ok'
    if name=='B4_above_cloud_bullish_div_or_recovering':
...
def run_window(name,start,end,histories):
    strategies=['A4_oversold_near_support_bullish_div','B4_above_cloud_bullish_div_or_recovering']
