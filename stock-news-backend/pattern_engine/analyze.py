"""
analyze.py — Bộ não trung tâm của engine.

Pipeline tự động cho MỘT mã:
  load -> infer_timeframe -> build_config (tự chỉnh tham số theo khung+độ dài)
       -> indicators -> pivots (distance theo config)
       -> detect tất cả pattern (lookback theo config)
       -> classify_and_rank (dedup, confluence, conflict, priority)
       -> forecast + scenarios
       -> trả về dict kết quả đầy đủ.

Dùng được cho bất kỳ CSV/JSON OHLCV nào, bất kỳ khung nào.
"""
...
def detect_all(df, pivots, cfg, include_experimental=True):
    """Gọi mọi detector với tham số lấy từ config."""
    P = []
...
def analyze(csv_or_df, symbol="STOCK", include_experimental=True):
    """Chạy toàn bộ pipeline, trả dict kết quả. csv_or_df: đường dẫn hoặc DataFrame đã load."""
    ...
    # key levels từ pattern (gần giá hiện tại)
