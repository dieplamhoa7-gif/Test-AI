...
OUT=Path('data/a4_bottom_reversal_no_divergence_backtest.json')
EXCLUDE={'VIC','VHM'}
HORIZON=42
...
def pass_strategy(name,rs,ai):
    ...
    if name=='A4_bottom_reversal_no_divergence':
        # User-requested bottom/reversal version: ignore BOTH bullish and bearish divergence.
        # Focus on low RSI, near support, low BB position, controlled volume, ROC not too damaged, MACD hist recovering.
...
def run_window(name,start,end,histories):
    strategies=['A4_bottom_reversal_no_divergence']
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'method':'A4 bottom reversal no divergence. Fixed exit target +6%, take 50%, trailing, stop 6%. Ignores both bullish and bearish divergence as requested. Conditions: no below_cloud, distSupport<=2.5, RSI<=40, BB<=0.45, volume 0.55-2.5, ROC20>=-12, MACD hist recovering 3 bars. R/S remains zone-only. Excludes VIC/VHM. Saves full entryIndicators and futurePath.','windows':{'current180':current,'prev3m':prev}}
