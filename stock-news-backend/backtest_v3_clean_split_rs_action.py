...
OUT=Path('data/v3_clean_split_rs_action_backtest.json')
EXCLUDE={'VIC','VHM'}
HORIZON=42
...
def pass_strategy(name,rs,ai):
    ...
    if name=='A_oversold_near_support':
        if dist>2.0: return False,'dist>2'
        ...
        if not (improving or mh>=-0.05 or ai.get('bullishDivergence')): return False,'macd_not_improving'
        return True,'ok'
    if name=='B_confirmed_rebound_above_cloud':
        if ichi!='above_cloud': return False,'not_above_cloud'
        if dist>2.5: return False,'dist>2.5'
        if not (50<=rsi<=60): return False,'rsi_not_50_60'
        if volr<0.55 or volr>2.2: return False,'vol_bad'
        if roc<-8 or roc>12: return False,'roc_bad'
        if not (mh>=0 or improving): return False,'macd_bad'
...
def run_window(name,start,end,histories):
    strategies=['A_oversold_near_support','B_confirmed_rebound_above_cloud']
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'method':'Clean split R/S vs action. R/S indicators only produce support/resistance/zone/dist/stop-target references. Action uses RSI/MACD/BB/volume/ROC/Ichimoku/divergence only. Excludes VIC/VHM. Saves full entryIndicators and futurePath.','windows':{'current180':current,'prev3m':prev}}
