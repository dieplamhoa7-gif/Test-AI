...
OUT = ROOT / "data" / "wyckoff_long_horizon_walkforward.json"
HORIZONS = [10, 20, 40]
FEATURES = [
    "springScore",
    "upthrustScore",
...
def eval_split(panel: list[dict], split_date: str, horizon: int, target: str) -> dict:
    ...
    top_long = sorted(coefs.items(), key=lambda kv: kv[1], reverse=True)[:6]
    top_short = sorted(coefs.items(), key=lambda kv: kv[1])[:6]
    # rank buckets in OOS
...
def main() -> None:
    panel = build_panel()
    result = {'rows': len(panel), 'features': FEATURES, 'targets': {}}
    targets = ['label_markup', 'label_markdown', 'label_range']
    for h in HORIZONS:
        result['targets'][f'h{h}'] = {}
        for target in targets:
            splits = [eval_split(panel, s, h, target) for s in SPLITS]
            valid = [s for s in splits if 'testAUC' in s]
            result['targets'][f'h{h}'][target] = {
                'splits': splits,
    ...
    for h in HORIZONS:
        preview[f'h{h}'] = {}
        for target in targets:
            preview[f'h{h}'][target] = result['targets'][f'h{h}'][target]['summary']
