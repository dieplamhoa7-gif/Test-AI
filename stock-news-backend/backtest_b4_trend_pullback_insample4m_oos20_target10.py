...
OUT = Path('data/b4_trend_pullback_insample4m_oos20_target10_backtest.json')
EXCLUDE = {'VIC', 'VHM'}
HORIZON = 42
...
def pass_b4(price, rs, ai):
    ...
    if ichi != 'above_cloud':
        return False, 'not_above_cloud'
    if dist > 3.0:
        return False, 'dist>3'
    if not (48 <= rsi <= 62):
        return False, 'rsi_not_48_62'
    if volr < 0.55 or volr > 2.2:
        return False, 'vol_bad'
    if roc < -8 or roc > 12:
        return False, 'roc_bad'
    if not (ai.get('macdHistRecovering') or (ai.get('bullishDivergence') and mh >= -0.05)):
        return False, 'no_macd_recovery_or_div'
    if bbp > 0.85:
...
def trade_manage_limited(df, i, max_end_idx=None):
    entry = f(df.iloc[i + 1].close)
    stop = entry * 0.94
    target = entry * 1.10
    end_idx = i + 2 + HORIZON
    if max_end_idx is not None:
        end_idx = min(end_idx, max_end_idx)
    future = df.iloc[i + 2:end_idx]
    if future.empty:
        return None
    size = 1.0
    realized = 0.0
    ...
    outcome = 'timeout'
    path = []
    for n, row in enumerate(future.itertuples(), 1):
        high = f(row.high)
        low = f(row.low)
        close = f(row.close)
        peak = max(peak, high)
        ...
        if (not half) and high >= target:
            ...
            size = 0.5
            half = True
            stop = max(entry * 1.005, peak * 0.97)
            event = 'take_half_target'
        if half:
            stop = max(stop, entry * 1.005, peak * 0.97)
        if low <= stop:
            realized += size * ((stop - entry) / entry * 100)
            ...
            exitd = str(row.time.date())
            event = (event + '+stop' if event else 'stop')
            path.append({'date': str(row.time.date()), 'open': r(row.open), 'high': r(high), 'low': r(low), 'close': r(close), 'stop': r(stop), 'event': event})
            break
        path.append({'date': str(row.time.date()), 'open': r(row.open), 'high': r(high), 'low': r(low), 'close': r(close), 'stop': r(stop), 'event': event})
    if size > 0:
        last = f(future.iloc[-1].close)
...
def run_window(name, start, end, histories, require_full_horizon=True):
    trades = []
    rejects = Counter()
    counts = {}
    for sym in TECHNICAL_UNIVERSE[:50]:
        ...
        max_i = len(df) - HORIZON - 2 if require_full_horizon else len(df) - 2
        c = {'loops': 0, 'trades': 0}
        end_idx = int(df.index[df['time'] < end].max()) + 1 if (df['time'] < end).any() else len(df)
        for i in range(100, max_i):
            t = df.iloc[i].time
            if t < start or t >= end:
                continue
            c['loops'] += 1
            ...
            if not ok:
                rejects[reason] += 1
                continue
            tr = trade_manage(df, i) if require_full_horizon else trade_manage_limited(df, i, end_idx)
            if tr:
                sup = f(rs.get('activeSupportDay') or rs.get('supportDay'))
                dist = (price - sup) / price * 100 if price and sup else 999
    ...
    return {'window': name, 'start': str(start.date()), 'end': str(end.date()), 'requireFullHorizon': require_full_horizon, 'summary': summ(trades), 'trades': trades, 'rejects': dict(rejects.most_common(20)), 'counts': counts}


def main():
    symbols = TECHNICAL_UNIVERSE[:50]
    histories = load_histories(symbols)
    valid_ends = [df.iloc[-1].time for df in histories.values() if df is not None and not df.empty]
    end = pd.Timestamp(max(valid_ends)) if valid_ends else pd.Timestamp(datetime.now())
    oos_start = None
    for df in histories.values():
        if df is not None and len(df) >= 20:
            cand = pd.Timestamp(df.iloc[-20].time)
            oos_start = cand if oos_start is None else max(oos_start, cand)
    if oos_start is None:
        oos_start = end - timedelta(days=30)
    insample_start = oos_start - timedelta(days=122)
    insample = run_window('insample4m', insample_start, oos_start, histories, require_full_horizon=True)
    oos20 = run_window('oos20', oos_start, end + timedelta(days=1), histories, require_full_horizon=False)
    payload = {
        'createdAt': datetime.now().isoformat(),
        'name': 'B4 Trend Pullback insample4m oos20 target10',
        'method': 'Fixed locked B4 rules. In-sample is ~4 calendar months before the latest 20 candles. OOS is the latest 20 candles.',
        'windows': {'insampleStart': str(insample_start.date()), 'oosStart': str(oos_start.date()), 'end': str(end.date()), 'oosCandles': 20},
        'exit': {'targetPct': 10, 'takeProfitAtTargetPct': 50, 'trailingAfterTarget': 'max(entry+0.5%, peak-3%)', 'stopPct': 6},
        'insample4m': insample,
        'oos20': oos20,
    }
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')
    print(json.dumps({'output': str(OUT), 'windows': payload['windows'], 'insample4m': insample['summary'], 'oos20': oos20['summary']}, ensure_ascii=False, indent=2))
