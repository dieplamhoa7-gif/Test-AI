...
def build_market_cache(macro: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    hourly_cache = read_json(DATA / "hourly_indicators_vn100_cache.json", {})
    for row in hourly_cache.get("items", []) if isinstance(hourly_cache, dict) else []:
        sym = str(row.get("ticker") or row.get("symbol") or "").upper()
        if not sym:
            continue
    ...
    for row in rs.get("items", []) if isinstance(rs, dict) else []:
        ...
        detail["technical"] = {
            ...
            "atr": detail.get("atr"),
            "vwapDay": detail.get("vwapDay"),
            "donchianHighDay": detail.get("donchianHighDay"),
            "donchianLowDay": detail.get("donchianLowDay"),
            "donchianMidDay": detail.get("donchianMidDay"),
            "marketStructureDay": detail.get("marketStructureDay"),
            ...
            "supportLevelsDay": detail.get("supportLevelsDay"),
            "resistanceLevelsDay": detail.get("resistanceLevelsDay"),
            "srStatusDay": detail.get("srStatusDay"),
            "pivotWeek": detail.get("pivotWeek"),
            "supportWeek": detail.get("supportWeek"),
            "resistanceWeek": detail.get("resistanceWeek"),
            "supportLevelsWeek": detail.get("supportLevelsWeek"),
            "resistanceLevelsWeek": detail.get("resistanceLevelsWeek"),
            "activeSupportWeek": detail.get("activeSupportWeek") or detail.get("supportWeek"),
            "activeResistanceWeek": detail.get("activeResistanceWeek") or detail.get("resistanceWeek"),
            "srStatusWeek": detail.get("srStatusWeek"),
            "pivotMonth": detail.get("pivotMonth"),
            "supportMonth": detail.get("supportMonth"),
            "resistanceMonth": detail.get("resistanceMonth"),
            "supportLevelsMonth": detail.get("supportLevelsMonth"),
            "resistanceLevelsMonth": detail.get("resistanceLevelsMonth"),
            "activeSupportMonth": detail.get("activeSupportMonth") or detail.get("supportMonth"),
            "activeResistanceMonth": detail.get("activeResistanceMonth") or detail.get("resistanceMonth"),
            "srStatusMonth": detail.get("srStatusMonth"),
        }
...
def build_html() -> None:
    ...
    function parseVpsQuote(item) {
      ...
      const ref = Number(item?.r || 0);
      if (!price) return null;
      let volume = Number(item?.totalVolume || 0);
      if (!volume) volume = Number(item?.lot || item?.lastVolume || 0) * 10;
      return { price, refPrice: ref, changePct: ref ? Math.round(((price - ref) / ref * 100) * 100) / 100 : Number(item?.changePc || 0), volume, openPrice: Number(item?.openPrice || 0), highPrice: Number(item?.highPrice || 0), lowPrice: Number(item?.lowPrice || 0), avgPrice: Number(item?.avePrice || price) };
    }
