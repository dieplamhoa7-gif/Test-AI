...
OUT = Path('data/b4_trend_pullback_support25_backtest.json')
EXCLUDE = {'VIC', 'VHM'}
HORIZON = 42
...
def main():
    ...
    payload = {'createdAt': datetime.now().isoformat(), 'name': 'B4 Trend Pullback support<=2.5', 'exit': {'targetPct': 6, 'takeProfitAtTargetPct': 50, 'trailingAfterTarget': 'max(entry+0.5%, peak-3%)', 'stopPct': 6}, 'current180': current, 'prev3m': prev}
