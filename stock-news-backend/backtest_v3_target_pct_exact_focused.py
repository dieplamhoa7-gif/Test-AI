...
OUT=Path('data/v3_target_pct_exact_focused_backtest.json')
...
def main():
    ...
    payload={'createdAt':datetime.now().isoformat(),'sample':'180 ngày gần nhất; TECHNICAL_UNIVERSE tối đa 50 mã; vào lệnh phiên kế tiếp sau tín hiệu; horizon 42 phiên; không trend, không ADX/DI; bullish divergence chỉ cộng điểm','method':'Exact path focused grid target +6/+7, score/distance/risk tight filters','topWinRate':results[:30],'topBalanced':balanced[:30],'counts':counts}
