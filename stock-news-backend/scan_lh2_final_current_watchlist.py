...
def main():
    ...
    for sym,df in hist.items():
        ...
        missing_details=[f"{m['name']}: hiß╗çn {m['value']} / cß║ºn {m['need']}" for m in missing]
        ...
        item={'symbol':sym,'date':str(pd.Timestamp(date).date()),'close':rr(close),'action':'BUY' if full else 'WATCH','rankScore':score,
              ...
              'hoverNote':('─Éß║ít ─æß╗º LH2 Final ΓÇö c├│ thß╗â mua theo hß╗ç thß╗æng.' if full else 'Ch╞░a mua: c├▓n thiß║┐u ' + '; '.join(missing_details[:8])),
