...
Cải tiến so với v4/v5:
  * Bộ chỉ báo mở rộng: thêm ADX14, RSI14, ATR%, distance-from-MA (extension),
    MACD hist, Bollinger-width percentile (squeeze), nearHigh252, MA50 alignment,
    market regime (index > MA50), và RS-RANK percentile (xếp hạng momentum
    cross-sectional theo ngày) thay cho band RS tuyệt đối phụ thuộc regime.
  * Walk-forward kiểu anchored: tối ưu trên IS (2023-2024), kiểm chứng OOS
    (2025-2026). Backtest in ra IS / OOS / FULL riêng để lộ overfit thật sự.
  * Exit tham số hóa: classic target/stop + failure-exit, hoặc ATR trailing.

KẾT LUẬN QUAN TRỌNG (đã chứng minh bằng grid search >300k tổ hợp):
  Mục tiêu winrate > 70% VÀ avg return > 5% ĐỒNG THỜI là KHÔNG ĐẠT ĐƯỢC một
  cách bền vững (>= ~12 lệnh) trên VN100 2023-2026 ở khung long-only breakout.
  Trần WR ở >=20 lệnh chỉ ~65-67%; con số 75%/6.2% của v5 chỉ là 12 lệnh
  (small-sample, hầu hết rơi vào sóng tăng 2023) -> overfit, 0 lệnh 2025-2026.
  v6 chọn cấu hình ROBUST nhất (dương cả IS lẫn OOS) thay vì ép một con số ảo.

Cách chạy:
  python build_lh2_v6.py                 # preset mặc định = BALANCED
  python build_lh2_v6.py --preset HIGH_FREQ
File data cần có cạnh script (hoặc trong ./data/): vn100_history_from_2023.json
"""
...
def find_data() -> Path:
    for p in (HERE / "vn100_history_from_2023.json",
        ...
        if p.exists():
            return p
    raise FileNotFoundError("Không tìm thấy vn100_history_from_2023.json cạnh script hoặc trong ./data/")

FEE = 0.5          # phí + slippage round-trip, %/lệnh
MIN_HOLD = 3       # số phiên tối thiểu trước khi cho stop/target khớp
HORIZON = 60       # số phiên tối đa quan sát sau entry
START = pd.Timestamp("2023-01-01")
END   = pd.Timestamp("2026-06-01")
IS_END = pd.Timestamp("2025-01-01")   # in-sample: 2023-2024 ; out-of-sample: 2025+

# Mỗi preset gồm bộ lọc entry + cấu hình exit. Tất cả ngưỡng được tìm bằng
# random search trên IS rồi kiểm OOS (xem README_LH2_V6.md).
PRESETS = {
    # ROBUST mặc định: ~24 lệnh, FULL WR ~58% / avg ~4.0%, dương cả IS lẫn OOS.
    "BALANCED": dict(
    ...
    # Nhiều lệnh hơn (~68), WR OOS cao hơn (~65%) nhưng avg thấp (~1.4%): ưu tiên tần suất.
    "HIGH_FREQ": dict(
...
def sim_exit(df, i, x):
    ...
    for n, (_, row) in enumerate(fut.iterrows(), 1):
        ...
        if hi > peak:
            peak = hi
        # failure exit (cắt sớm khi breakout hỏng)
...
def run(preset_name: str):
    ...
    # market lookups by date (nhanh hơn .loc trong vòng lặp)
    mret = mkt["mktRet20"].to_dict(); mbr = mkt["breadth"].to_dict(); mreg = mkt["regime"].to_dict()
    trades, today = [], []
    candidates = 0
    for sym, df in hist.items():
        n = len(df)
        # arrays cột (tránh iloc trong hot-loop)
        ...
        for i in range(200, n - HORIZON - 2):
            ...
            if date < START or date >= END or date not in mret:
                continue
            row = type("R", (), {c: A[c][i] for c in A})()  # row-like với thuộc tính cột
