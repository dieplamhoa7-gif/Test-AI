...
payload={
    'symbol':'MWG','asOfDate':str(df.iloc[-1].time),'asOfPrice':float(df.iloc[-1].close),'createdAt':'2026-05-28T09:01:00+00:00',
    'summary':{'trendlines':len(selected),'parallelChannels':0,'pitchforks':0,'linregChannels':0,'srLevels':0,'patterns':0,'candlestickSignals':0,'currentBias':'touchzone-0.5pct'},
...
out=Path('firebase_public/data/charts/MWG_touchzone_day.json')
out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')
print(out)
print('selected', len(selected))
for l in selected: print(l['kind'], 'touches', l['touches'], 'breaks', l['breaks'], 'score', round(l['score'],1), 'from', l['points'][0]['time'], 'to', l['points'][1]['time'])
